...
function barListPrototype:NewBarFromPrototype(prototype, ...)
	...
	bar:SetBackgroundTexture(self.bgtexture)
...
function barListPrototype:SetBackgroundTexture(tex)
	self.bgtexture = tex
	if bars[self] then
		for k, v in pairs(bars[self]) do
			v:SetBackgroundTexture(tex)
		end
	end
end
...
function barPrototype:SetBackgroundTexture(texture)
